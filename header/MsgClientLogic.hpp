...
#define REQ_BL_ADD "REQ_BL_ADD"
#define REQ_BL_DEL "REQ_BL_DEL"
#define REQ_BL_LIST "REQ_BL_LIST"
...
inline int net_send(sock_t sock, const char *body)
{
    ...
    if (send(sock, (const char *)&net_len, 4, 0) != 4)
        return -1;
    uint32_t sent = 0;
    while (sent < body_len)
    {
...
inline int net_recv(sock_t sock, char *out_buf, int buf_size)
{
    ...
    if (body_len == 0 || (int)body_len >= buf_size)
        return -1;
    uint32_t got = 0;
    while (got < body_len)
    {
...
#endif
{
    ...
    while (1)
    {
        ...
        if (net_send(g_sock, body) < 0)
            continue;
        if (net_recv(g_sock, res, sizeof(res)) < 0)
            continue;
...
inline void menu_blacklist()
{
    ...
    while (1)
    {
        ...
        if (choice == 1)
        {
            ...
            char esc[512], body[512];
            json_escape(email, esc, sizeof(esc));
            snprintf(body, sizeof(body),
                     "{\"type\":\"%s\",\"user_pk\":%d,\"target_email\":\"%s\"}",
                     REQ_BL_ADD, g_user_pk, esc);
        ...
        else if (choice == 2)
        {
            ...
            char body[256];
            snprintf(body, sizeof(body),
                     "{\"type\":\"%s\",\"user_pk\":%d}", REQ_BL_LIST, g_user_pk);
            ...
            // blacklist 배열 간단 파싱 출력
            printf("\n현재 차단 목록:\n");
            char *p = strstr(res, "\"blacklist\":");
            if (!p || strstr(p, "[]"))
            {
                printf("  차단된 유저가 없습니다.\n");
                printf("계속하려면 Enter...\n");
            ...
            // 각 항목에서 blacklist_num, email 추출해서 출력
            int idx = 1;
            char *cur = p;
            while ((cur = strstr(cur, "\"blacklist_num\":")) != NULL)
            {
                int bl_num = 0;
                sscanf(cur, "\"blacklist_num\":%d", &bl_num);
                char em[128] = {0};
                char *ep = strstr(cur, "\"email\":\"");
                if (ep)
                {
                    sscanf(ep, "\"email\":\"%127[^\"]\"", em);
                }
                printf("  [%d] bl_pk=%d  %s\n", idx++, bl_num, em);
                cur++;
            }
            ...
            fflush(stdout);
            char pk_input[16] = {0};
            fgets(pk_input, sizeof(pk_input), stdin);
            int bl_pk = atoi(pk_input);

            snprintf(body, sizeof(body),
                     "{\"type\":\"%s\",\"user_pk\":%d,\"blacklist_num\":%d}",
                     REQ_BL_DEL, g_user_pk, bl_pk);
        ...
        else if (choice == 3)
        {
            ...
            {
                char *cur2 = p2;
                while ((cur2 = strstr(cur2, "\"blacklist_num\":")) != NULL)
                {
                    int bl_num = 0;
                    sscanf(cur2, "\"blacklist_num\":%d", &bl_num);
                    char em[128] = {0}, ca[32] = {0};
                    char *ep = strstr(cur2, "\"email\":\"");
                    if (ep)
                        sscanf(ep, "\"email\":\"%127[^\"]\"", em);
                    char *cp = strstr(cur2, "\"created_at\":\"");
                    if (cp)
                        sscanf(cp, "\"created_at\":\"%31[^\"]\"", ca);
                    printf("  [bl_pk:%d] %s  (%s)\n", bl_num, em, ca);
                    cur2++;
                }
            }
